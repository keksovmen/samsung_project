...
class SwitchDataViewFragment : Fragment() {
    ...
    private lateinit var parent: Switch
    ...
    override fun onCreateView(
        ...
        parent = requireArguments().getSerializable(Globals.ARGUMENT_PARENT) as Switch
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        database.flowerBoxDao().getAllSwitchData(parent.parent_id, parent.id).observe(
    ...
    private fun onSyncClicked(_ignore: View) {
        Globals.executor.submit {
            val timestamp = database.flowerBoxDao().getLastSwitchData(parent.parent_id, parent.id)

            val time = (timestamp?.timestamp ?: (Date().time / 1000)) - 60 * 60 * 24
            val api = BoxApi(box.ip, box.port)
            api.fetchSwitchData(parent, time) { vals: List<DataEntry>? ->
                if (vals == null) {
            ...
            LinearListAdapter(
                ...
                { (value, timestamp): DataEntry, view: View ->
                    (view.findViewById<View>(R.id.date) as TextView).text =
                        Globals.formatDataView.format(
                            Date(
                                timestamp * 1000
                            )
                        )
                    (view.findViewById<View>(R.id.value) as TextView).text =
                        if (value == 1.0f) "ON" else "OFF"
                },
